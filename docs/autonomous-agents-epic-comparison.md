...
**Author**: [Stas](https://stas.vision) — Emberloom is a portfolio/learning project I built to explore autonomous agent architecture. This comparison documents what I learned about the landscape and where Emberloom fits relative to production tools backed by well-funded teams.
...
- Emberloom is one of the products being compared. Its ratings are based on source code inspection but readers should weight external validation more heavily.
...
|-------|---------|------|-----------|-------|---------|
| **Emberloom** | Emberloom | Self-hosted multi-agent system | Not published | Self-hosted | Source-available |
...
| Capability | Emberloom | Pilot | Devin | OpenHands | Cursor | Copilot | Claude Code | Codex | Aider |
|------------|--------|-------|-------|-----------|--------|---------|-------------|-------|-------|
...
Emberloom and Pilot lead on autonomous execution. Most agents now offer some form of bounded autonomy — from binary flags (Aider's `--yes`) to composable knobs (Emberloom) to permission modes (Claude Code). Self-healing remains basic across the field, with error-loop retry being the most common pattern.
...
**Auto-label monitoring** — Can the agent watch for a specific label (e.g., `emberloom`, `@copilot`) and start work automatically when applied? This is the difference between "assign to agent" and "agent finds its own work."
...
| Capability | Emberloom | Pilot | Devin | OpenHands | Copilot | Factory | Codex | Sweep | Jules |
|------------|--------|-------|-------|-----------|---------|---------|-------|-------|-------|
...
| Code generation | **Strong** (multi-spark) | **Strong** (Claude Code) | **Strong** | **Strong** (full-stack) | **Strong** | **Strong** (Code Droid) | **Strong** (GPT-5 Codex) | **Good** | **Strong** |
...
Pilot leads the ticket-to-PR pipeline with Autopilot CI, 8-platform ticket intake, and auto-merge. GitHub Copilot's native `@copilot` issue assignment is the most frictionless entry point. Emberloom has strong planning (feature contracts with DAG ordering) but lacks automated ticket pickup and CI monitoring.
...
**Multiple agent personas** — Can the system run different agents with different skills, tools, or system prompts? Examples: Emberloom's sparks (coder, scout, custom), Devin's Planner/Coder/Critic, Factory's Droids. Agents with configurable personas score higher than fixed roles.

**Parallel agent execution** — Can multiple agents work simultaneously on different tasks or subtasks? This is the difference between sequential task processing and true parallelism. Implementations vary: git worktrees (Conductor, Codex), Docker containers (Emberloom), cloud VMs (Devin), or in-process threads (Claude Code subagents).
...
**Spark/agent routing** — How does the system decide which agent handles a task? Simple: user specifies. Good: rule-based routing. Strong: classifier model or coordinator agent that analyzes the task and delegates.
...
|------------|--------|-------|-------|-----------|---------|-----------|-------------|-------|--------|--------|
| Multiple agent personas | **Strong** (sparks: coder, scout, custom) | -- | **Strong** (Planner, Coder, Critic) | **Good** (configurable) | **Strong** (4 Droids) | -- | **Good** (subagents) | **Good** (via Skills) | **Strong** (coordinator + specialist agents) | -- |
...
**Semantic memory (embeddings)** — Does the agent store memories as vector embeddings for similarity search? This enables "find memories similar to X" rather than exact keyword matching. Emberloom uses ONNX 384-dimensional embeddings with cosine similarity. No other surveyed self-hosted agent implements this.

**Long-term memory** — Can the agent persist knowledge across sessions? Implementations: SQLite databases (Emberloom), flat files (CLAUDE.md), project-level learning (Devin), or codebase indexes (Cursor). The key question is whether the memory is structured and searchable vs. a simple text dump.

**Recency decay** — Do older memories lose relevance over time? Emberloom implements configurable half-life decay so recent context is weighted more heavily in search results. Without decay, memory databases grow without bound and old irrelevant context pollutes results.

**Deduplication** — When storing new memories, does the agent detect and merge duplicates? Emberloom uses cosine similarity thresholds — if a new memory is too similar to an existing one, it updates rather than duplicates. Without deduplication, agents accumulate redundant context.
...
**Relationship tracking** — Does the agent track per-user interaction patterns (topics discussed, communication preferences, warmth)? Emberloom has the schema but sentiment computation is not fully implemented.

| Capability | Emberloom | Pilot | Devin | OpenHands | Augment | Aider | Claude Code | Codex | Intent | Cursor |
|------------|--------|-------|-------|-----------|---------|-------|-------------|-------|--------|--------|
...
Emberloom has the most sophisticated memory architecture among self-hosted agents (embedding search, FTS5, recency decay, deduplication). However, Augment Code's context engine indexes 500K+ files across multiple repos — an area where Emberloom has no equivalent. Relationship tracking exists in Emberloom's schema but sentiment computation is not yet fully implemented.
...
**Container hardening** — Beyond basic Docker, does the agent apply security hardening? Emberloom drops all Linux capabilities (`CAP_DROP ALL`), uses a read-only root filesystem, disables networking, limits PIDs to 256, and mounts `/tmp` as `tmpfs` with `noexec`. This is defense-in-depth — even if the LLM generates malicious code, the sandbox limits the blast radius.

**Tool safety validation** — Does the agent validate tool inputs before execution? Emberloom checks for path traversal (`..`), SSRF (blocks `localhost`, private IPs), and sensitive file access (`.env`, `*.pem`, `credentials.json`). Claude Code uses a permission system that prompts before file writes. Most agents have no input validation.

**CLI tool integration** — Can the agent delegate work to other CLI agents (Claude Code, Codex, opencode)? This is an Emberloom-specific pattern where the orchestrator dispatches tasks to external agents running inside its Docker sandbox.
...
**Deployment options** — Where can the agent run? Options: local binary, Docker, Kubernetes, cloud VMs, SaaS-only. Self-hosted agents (Emberloom, Pilot, Aider) offer the most flexibility. Cloud-only agents (Devin, Copilot's coding agent) require data to leave your environment.

| Capability | Emberloom | Pilot | Devin | OpenHands | Copilot | Claude Code | Codex | Aider | Cursor |
|------------|--------|-------|-------|-----------|---------|-------------|-------|-------|--------|
...
Emberloom has the most hardened sandbox configuration (CAP_DROP ALL + SSRF + path traversal + PID limits + readonly rootfs). No other self-hosted agent combines all these measures. Pilot leads on deployment flexibility and hot-upgrade capability. Auto-update is now common across app-based agents (Codex, Cursor).
...
**Real-time event stream** — Does the agent emit structured events as it works? Emberloom streams 18 event types (startup, mood change, tool usage, pulse delivery, etc.) via a Unix domain socket. OpenHands has an event log. Most agents provide only final output.

**Langfuse integration** — Does the agent send traces, spans, and generation metadata to an observability platform? Emberloom integrates with Langfuse for trace-level visibility into LLM calls, tool executions, and background task pipelines. No other self-hosted agent has this.

**KPI tracking** — Does the agent track its own performance metrics (task success rate, verification pass rate, mean time to fix)? Emberloom segments KPIs by lane (delivery vs. self-improvement), repo, and risk tier.

**Health diagnostics** — Can the agent diagnose its own configuration and health? Emberloom's `doctor` command runs 4 diagnostic funnels checking LLM connectivity, proactive feature wiring, memory pipeline health, and execution environment readiness.

**Introspection (self-metrics)** — Does the agent monitor its own process metrics (RSS memory, CPU, error rate, LLM latency)? Emberloom collects these and triggers anomaly detection when thresholds are exceeded.
...
| Capability | Emberloom | Pilot | Devin | OpenHands | Factory | Claude Code | Codex | Copilot | Cursor | Aider |
|------------|--------|-------|-------|-----------|---------|-------------|-------|---------|--------|-------|
...
Emberloom has the deepest observability stack among self-hosted agents (event streaming, Langfuse tracing, KPI tracking, health diagnostics, per-tool statistics). Most agents provide at least basic cost visibility and streaming output. The gap is visual presentation — Emberloom lacks a dashboard while competitors offer web UIs, TUIs, or threaded conversation interfaces.
...
**Feature contracts (DAG)** — Can the agent define tasks as a directed acyclic graph with dependencies? Emberloom implements topological sort via Kahn's algorithm with cycle detection and dependency validation. This ensures tasks execute in the correct order. No other surveyed agent has this.
...
**Acceptance criteria** — Can the agent map explicit success criteria to tasks? Emberloom's feature contracts define acceptance criteria and validate that every criterion is covered by at least one task.

**Verification profiles** — Can the agent run different levels of verification (fast smoke test vs. strict full test suite)? Emberloom supports `fast` and `strict` profiles. Pilot uses configurable quality gates.
...
| Capability | Emberloom | Pilot | Devin | Conductor | Copilot | Claude Code | Codex | Intent | Augment | Cursor |
|------------|--------|-------|-------|-----------|---------|-------------|-------|--------|---------|--------|
...
Emberloom has the strongest planning primitives (feature contracts with DAG ordering, acceptance criteria, verification profiles). Codex offers built-in diff review with inline commenting and auto-commit checkpoints. Intent introduces spec-driven development where living specifications guide agent work. Conductor leads on interactive review workflows.
...
**Slack / Telegram** — Can the agent receive tasks and send updates via messaging platforms? Emberloom's Telegram integration includes a multi-step planning interview with inline keyboards. Most agents have no messaging integration.
...
**IDE integration** — Can the agent work inside an editor? Ranges from VS Code extensions to native IDEs (Cursor, Windsurf). CLI-only agents (Emberloom, Aider) have no IDE integration.
...
| Capability | Emberloom | Pilot | Devin | Copilot | Factory | Cursor | Claude Code | Codex | Intent | Aider |
|------------|--------|-------|-------|---------|---------|--------|-------------|-------|--------|-------|
...
Emberloom's Telegram integration is unique (planning interviews with inline keyboards), but its broader integration surface is thin. Codex's Skills library (Figma, Linear, Vercel, Cloudflare) and MCP support give it broad extensibility. Pilot and Factory lead with multi-platform support. GitHub Copilot and Cursor have the deepest IDE integration.
...
**Interactive chat** — Can the developer have a back-and-forth conversation with the agent? CLI chat, IDE sidebar, web UI, or messaging app. Agents with multiple interaction modes (Emberloom: CLI + Telegram) score higher.
...
**Voice input** — Can the developer speak to the agent? Emberloom supports voice via Telegram's speech-to-text. Codex has built-in voice dictation. Rare feature.
...
**Configuration depth** — How many runtime parameters can be tuned? Emberloom has 50+ knobs (spontaneity, quiet hours, heartbeat interval, mood drift, etc.). Most agents have a settings file with 5-10 options.
...
| Capability | Emberloom | Pilot | Devin | Cursor | Copilot | Claude Code | Codex | Intent | Aider |
|------------|--------|-------|-------|--------|---------|-------------|-------|--------|-------|
...
IDE-based tools (Cursor, Copilot) have the lowest adoption friction. Codex's macOS app offers a polished middle ground between IDE and CLI. Intent provides an all-in-one workspace (editor + terminal + browser preview) but is macOS-only and in beta. Emberloom has the deepest configuration system (50+ runtime knobs).
...
**Container hardening** — Beyond running in Docker, does the agent apply security best practices? Measures include: dropping Linux capabilities, read-only filesystems, PID limits, memory limits, network isolation, and running as non-root. Emberloom applies all of these.

**Path traversal protection** — Does the agent prevent LLM-generated code from accessing files outside the workspace (e.g., `../../etc/passwd`)? Emberloom validates all paths and rejects `..` traversal. Codex scopes directory access to the current project.

**SSRF protection** — Does the agent prevent the LLM from making requests to internal network addresses (localhost, private IPs)? Emberloom blocks all private IP ranges, IPv6 loopback, and link-local addresses. Codex disables network access by default.

**Sensitive file blocking** — Does the agent prevent access to secrets and credentials (`.env`, `*.pem`, `credentials.json`)? Emberloom has a regex-based blocklist.

**SOC 2 / compliance certs** — Does the vendor have enterprise compliance certifications? Amazon Q (HIPAA, SOC2) and Factory (SOC2, GDPR, ISO) lead here. Self-hosted open-source agents (Emberloom, Aider) have no certifications but offer data sovereignty.
...
| Capability | Emberloom | Pilot | Devin | OpenHands | Copilot | Amazon Q | Factory | Claude Code | Codex | Cursor | Aider |
|------------|--------|-------|-------|-----------|---------|----------|---------|-------------|-------|--------|-------|
...
Emberloom has the deepest technical security hardening (container + input validation). Amazon Q and Factory lead on compliance certifications. Self-hosted tools (Emberloom, Pilot, OpenHands, Aider, Claude Code) offer the strongest data privacy guarantees — Aider is particularly notable since it runs locally with any LLM provider.
...
**Mood system** — Does the agent simulate emotional state that affects its behavior? Emberloom models energy (0-1, with a time-of-day curve peaking at 9-11am) and valence (positive/negative), plus 10 personality modifiers (curious, focused, playful, contemplative, etc.). The mood description is injected into system prompts.

**Idle musings & conversation re-entry** — Does the agent proactively follow up after a conversation ends? Emberloom samples random memories when idle, generates reflections via LLM, and can schedule follow-up messages based on past context.

**Cron/interval scheduling** — Can the agent run tasks on a schedule without human triggers? Emberloom implements POSIX cron, interval-with-jitter, and one-shot scheduling. Codex's Automations feature is the closest competitor — it runs instructions on a defined schedule with results landing in a review queue.
...
| Capability | Emberloom | Codex | Claude Code | Cursor | Others |
|------------|--------|-------|-------------|--------|--------|
...
|-------|--------------|-----------|---------------------|---------------|-----------------|-------------|----------------|--------------|----------------------|------------------|---------------------|-------------------|
| **Emberloom** | 5 (10) | 7 (10.5) | 3 (4.5) | 7 | 8 | 8 | 9 | 7 | 9 (6.75) | 8 (6) | 9 (4.5) | **82.25** |
...
**Reading the scores**: Emberloom (82.25), Pilot (77.5), and Codex (70.75) lead overall but for different reasons — Emberloom through depth in autonomy, memory, execution, and observability; Pilot through the pipeline and integrations; Codex through broad DX, integrations (Skills + MCP), and parallel agents. Copilot (68.5), Factory (66), and Devin (65.25) form a competitive middle tier. Intent (56.5) is early but its spec-driven multi-agent approach is architecturally interesting.
...
|-------|-------------------|
| **Emberloom** | Semantic memory with ONNX embeddings + recency decay; hardened Docker sandbox (CAP_DROP ALL, SSRF/path-traversal protection); Langfuse observability; cron scheduling with quiet hours; mood/personality system |
