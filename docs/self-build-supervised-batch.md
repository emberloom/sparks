...
By default the runner pre-builds `target/debug/sparks` before executing tickets.
Use `--skip-build` only when you intentionally want to reuse an existing binary.
`sparks self-build run` now retries one time automatically when dispatch reports success but leaves an empty git diff.
